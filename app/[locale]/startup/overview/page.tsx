...
export default function StartupOverviewPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      const [s, p, d, bp] = await Promise.all([
        safe(() => getDocs(query(collection(db, "startups"), where("owner_uid", "==", user.uid)))),
        safe(() => getDoc(doc(db, "profiles", user.uid))),
        safe(() => getDoc(doc(db, "pitchdecks", user.uid))),
        safe(() => getDoc(doc(db, "businessplans", user.uid))),
      ]);
      if (s) setStartups(s.docs.map((doc) => ({ id: doc.id, ...(doc.data() as Omit<StartupDoc, "id">) })));
  ...
  const isProPlus = isStartupProPlus({ plan_tier: tier, capabilities: caps });
    ...
    <div className="min-h-screen bg-zinc-50 pb-20">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <div className="flex items-start justify-between gap-4">
          ...
          {startups.length > 0 && (
            <Link href={`/${locale}/startup/profile`} className="shrink-0 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
              + {t("new_startup")}
            </Link>
          )}
        </div>
        ...
        {startups.length === 0 ? (
          ...
          <div className="mt-6 flex flex-col gap-4">
            {startups.map((startup) => {
              ...
              const complete = !!(startup.name && startup.tagline && startup.description);
              const showFunding = paid && startup.is_discoverable;
              return (
                <section key={startup.id} className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-xl font-extrabold text-zinc-900">{startup.name || t("untitled")}</p>
                        {isProPlus && (
                        ...
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${complete ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
                          {complete ? t("complete") : t("incomplete")}
                        </span>
                      </div>
                      {startup.tagline && <p className="mt-0.5 text-zinc-600">{startup.tagline}</p>}
                    </div>
                    <div className="flex shrink-0 gap-2">
                      <Link href={`/${locale}/startup/profile?id=${startup.id}`} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50">
                        {t("edit")}
                      </Link>
                      <Link href={`/${locale}/company/${startup.id}`} target="_blank" className="rounded-lg bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-700">
                        {t("view_public_page")}
                      </Link>
                    </div>
                  </div>
                  ...
                  {startup.description && <p className="mt-3 text-sm leading-relaxed text-zinc-600">{startup.description}</p>}
                  ...
                  <div className="mt-3 rounded-xl border border-zinc-100 bg-zinc-50 p-4">
                    ...
                    <div className="space-y-1 text-sm text-zinc-600">
                      ...
                      {!showFunding && paid && <p className="text-xs text-zinc-400">🔒 {t("funding_hidden")}</p>}
                    </div>
                  </div>
        ...
        {/* Pitchdeck + Business plan tiles (per user) */}
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          <DocTile
            emoji="🎯" title={t("pitchdeck")}
            href={`/${locale}/startup/pitchdeck`}
            status={hasDeck ? t("complete") : t("not_created")}
            ok={hasDeck}
            desc={t("pitchdeck_internal")}
            cta={hasDeck ? t("open") : t("create_cta")}
          />
          <DocTile
            emoji="📄" title={t("businessplan")}
            href={`/${locale}/startup/plan`}
            status={plan ? t("complete") : t("not_saved_yet")}
            ok={!!plan}
            desc={plan ? (plan.internal?.coreIdea || t("plan_internal")) : t("plan_internal")}
