...
const ACCENT: Record<string, { ring: string; btn: string; badge: string }> = {
  free:  { ring: "",                       btn: "bg-zinc-900 text-white hover:bg-zinc-700",       badge: "" },
  plus:  { ring: "ring-2 ring-indigo-400", btn: "bg-indigo-600 text-white hover:bg-indigo-700",   badge: "bg-indigo-100 text-indigo-700" },
  pro:   { ring: "ring-2 ring-amber-400",  btn: "bg-amber-500 text-white hover:bg-amber-600",     badge: "bg-amber-100 text-amber-700" },
};
  ...
  async function handleUpgrade(tierId: string) {
    ...
    try {
      const res = await fetch("/api/billing/checkout", {
        ...
        body: JSON.stringify({ priceId, successUrl: "/talent", cancelUrl: "/talent/billing" }),
      });
    ...
    <div className="min-h-screen bg-zinc-50 pb-20">
      <header className="border-b border-zinc-200 bg-white px-6 py-4">
        <div className="mx-auto flex max-w-4xl items-center gap-4">
          ...
          <h1 className="text-lg font-semibold text-zinc-900">Talent-Plan</h1>
        </div>
      </header>
              ...
              <div key={tier.id} className={`relative rounded-2xl border border-zinc-200 bg-white p-7 shadow-sm flex flex-col ${a.ring}`}>
                ...
                <div className="mt-1 flex items-baseline gap-1 mb-5">
                  {tier.price === 0
                    ...
                    : <><span className="text-2xl font-black text-zinc-900">{tier.price} €</span><span className="text-sm text-zinc-400">/ Mo</span></>}
                </div>
